...
if (count($_POST) > 0) {
    $result = mysqli_query($conn, "SELECT * FROM tbl_userAdmin WHERE admin_email='" . $_POST["admin_email"] . "'");
    $row  = mysqli_fetch_array($result);
    if (is_array($row)) {
        echo "<script>window.open('reset_password','_self')</script>";
        $_SESSION["admin_email"] = $row["admin_email"];
    } else {
       echo '<script>alert("This email does exists");</script>';
    }
}
